...
/**
 * Application for selecting a shape from the Monstrous Physique spell to change into and then applying that shape to an actor
 */
export class MorphinMonstrousPhysique extends MorphinPolymorphDialog {
    /**
     * @inheritdoc
    ...
    constructor(level, durationLevel, actorId, source, nameOverride, {planarType = null, energizedTypes = null, mutatedType = null} = {}) {
        ...
        switch (level) {
            ...
            case 3:
                ...
                break;
            case 2:
                this.sizes.monstrous = ['tiny', 'sm', 'med', 'lg'];
                break;
            case 1:
                this.sizes.monstrous = ['sm', 'med'];
                break;
        }

        this.shapeOptions = {};
        // Find options to shapeshift into based on the valid size choices above and sort them alphabetically
        this.shapeOptions.monstrous = MorphinOptions.monstrous.filter(o => this.sizes.monstrous.includes(o.size));
        this.shapeOptions.monstrous.sort((a, b) => { return a.name > b.name ? 1 : -1; });
    ...
    static get defaultOptions() {
        return mergeObject(super.defaultOptions, {
            ...
            popOut: true,
            template: 'modules/pf1-mighty-morphin/templates/monstrousPhysiqueDialog.html',
            id: 'mighty-morphin-monstrousPhysique',
            title: game.i18n.localize('MMMOD.UI.MonstrousDialogName'),
            resizable: false,
            width: 550
        });
    }
    ...
    async getData() {
        ...
        // Set the default size to the largest available monstrous humanoid (default type)
        let defaultSize = this.sizes.monstrous[this.sizes.monstrous.length - 1];
        data.monstrousOptions = this.shapeOptions.monstrous.filter(o => o.size === defaultSize);
        // Create radio button data for monstrous sizes, set the one for the default size as the default checked button
        data.monstrousSizes = this.sizes.monstrous.map(o => { return o === defaultSize ? { label: o, size: CONFIG.PF1.actorSizes[o], default: true } : { label: o, size: CONFIG.PF1.actorSizes[o] }; });

        // Get the monstrous humanoid that will be the first shown in the form dropdown and build the preview of the changes the form will provide
        data.defaultChoice = data.monstrousOptions[0];
        data.previewHtml = await this.buildPreviewTemplate(data.defaultChoice.name, 'monstrous');
    ...
    async updateFormChoices(event, formSelect) {
        let newOptions = this.shapeOptions.monstrous.filter(o => o.size === event.target.value);
    ...
    /**
     * Processes the changes from the selected monstrous humanoid form into a readable preview display html. In the process it validates the changes based on the level of the spell
     * 
